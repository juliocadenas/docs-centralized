...
class GPUManager:
    ...
    def __init__(self):
        ...
        self._last_used: Dict[str, Optional[float]] = {}
        self._watchdog_task: Optional[asyncio.Task] = None
        # Semaphore: only 1 GPU-intensive job at a time
    ...
    async def start_watchdog(self):
        ...
        logger.info("GPU watchdog started - idle services stopped after %ds", IDLE_TIMEOUT_SECONDS)
    ...
    def _query_gpu_sync(self) -> Optional[Dict]:
        """Synchronous nvidia-smi query (runs in thread pool)."""
    ...
    async def get_gpu_info(self) -> Dict:
        """Get GPU info (non-blocking, runs nvidia-smi in thread pool)."""
        try:
            result = await asyncio.to_thread(self._query_gpu_sync)
            if result:
                return result
            logger.warning("nvidia-smi not found or returned no data.")
            return {
    ...
    async def start_service(self, service_name: str) -> Dict:
        ...
        return {"status": "no_action", "message": f"{service['name']} has no start method"}
    ...
    async def stop_service(self, service_name: str) -> Dict:
        ...
        return {"status": "no_action", "message": f"{service['name']} has no stop method"}
    ...
    async def _start_docker_service(self, service_name: str, display_name: str) -> Dict:
        try:
            result = await asyncio.to_thread(
                ...
                **{"capture_output": True, "text": True, "timeout": 60, "cwd": DOCKER_COMPOSE_DIR},
            )
            if result.returncode == 0:
                return {"status": "started", "message": f"{display_name} started successfully"}
