...
# Map OpenAI voice names to Piper voice models
VOICE_MAP = {
    ...
    "nova": "es_ES-sharvard-medium",
    "shimmer": "es_ES-Beatriz-medium",
}
...
async def create_speech(request: SpeechRequest):
    ...
    try:
        voice_model = VOICE_MAP.get(request.voice or "es", "es_ES-sharvard-medium")
...
async def create_transcription(
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            resp = await client.post(f"{WHISPER_STT_URL}/transcribe", files=files, data=data)
            if resp.status_code != 200:
