...
VOICES_INFO = {
    "es_ES-sharvard-medium": "Sharvard (ES neutro)",
    "es_ES-Beatriz-medium": "Beatriz",
    "es_ES-Davefx-medium": "Davefx",
    "en_US-amy-medium": "Amy (US)",
    "en_GB-alan-medium": "Alan (UK)",
    "fr_FR-siwis-medium": "Siwis (FR)",
    "de_DE-thorsten-medium": "Thorsten (DE)",
}

def find_onnx(voice_id):
    """Find .onnx file for voice in DATA_DIR."""
    # Pattern: es_ES-sharvard-medium -> es_ES-sharvard-medium.onnx
...
sftp.close()
print("tts_svc.py subido (piper 1.4.2 API)")
...
# Test TTS (primera vez descarga modelo, tarda)
print("\nTest TTS (descarga voz + genera, 30-60s)...")
_, o, _ = ssh.exec_command(
    "curl -s -X POST http://localhost:8010/api/tts -F 'text=Hola mundo' -F 'voice=es_ES-sharvard-medium' -o /tmp/tts_test.wav -w '%{http_code}' --max-time 120 2>/dev/null",
    timeout=180)
print(f"HTTP: {o.read().decode().strip()}")

_, o, _ = ssh.exec_command('ls -lh /tmp/tts_test.wav 2>/dev/null; file /tmp/tts_test.wav 2>/dev/null')
print(f"Audio: {o.read().decode('utf-8','replace').strip()}")

# Logs
_, o, _ = ssh.exec_command('echo pepe1234 | sudo -S journalctl -u tts --no-pager -n 20 2>&1')
print(f"\nLogs:\n{o.read().decode('utf-8','replace').strip()[-800:]}")

# STT
_, o, _ = ssh.exec_command('curl -s http://localhost:8020/api/status 2>/dev/null')
print(f"\nSTT: {o.read().decode().strip()}")

# GPU
_, o, _ = ssh.exec_command('nvidia-smi --query-gpu=memory.used,memory.total --format=csv,noheader 2>/dev/null')
